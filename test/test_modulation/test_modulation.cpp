...
namespace {
    const BipolarRange<s0x16> &SIGNED_RANGE = bipolarRange();
}
...
void test_sine_waveform() {
    ...
    TEST_ASSERT_INT32_WITHIN(500, S0X16_MAX, raw(s.sample(SIGNED_RANGE, 250)));
    
    (void) s.sample(SIGNED_RANGE, 400);
    (void) s.sample(SIGNED_RANGE, 600);
    // t=750ms -> -1.0 (S0X16_MIN)
    TEST_ASSERT_INT32_WITHIN(500, S0X16_MIN, raw(s.sample(SIGNED_RANGE, 750)));
}

/** @brief Verify triangle waveform. */
void test_triangle_waveform() {
    S0x16Signal s = triangle(constant(1000), s0x16(0));
    
    // t=0 -> Initialize, returns -1
    TEST_ASSERT_INT32_WITHIN(100, S0X16_MIN, raw(s.sample(SIGNED_RANGE, 0)));
    ...
    (void) s.sample(SIGNED_RANGE, 400);
    // t=500ms -> 1
    TEST_ASSERT_INT32_WITHIN(100, S0X16_MAX, raw(s.sample(SIGNED_RANGE, 500)));
}

/** @brief Verify square waveform. */
void test_square_waveform() {
    S0x16Signal s = square(constant(1000), s0x16(0));
    
    // t=0 -> Initialize
    (void) s.sample(SIGNED_RANGE, 0);

    // t=100ms -> 1.0 (phase < 0.5)
    TEST_ASSERT_INT32_WITHIN(10, S0X16_MAX, raw(s.sample(SIGNED_RANGE, 100)));
    
    (void) s.sample(SIGNED_RANGE, 300);
    (void) s.sample(SIGNED_RANGE, 500);
    // t=600ms -> -1.0 (phase > 0.5)
    TEST_ASSERT_INT32_WITHIN(10, S0X16_MIN, raw(s.sample(SIGNED_RANGE, 600)));
}

/** @brief Verify sawtooth waveform. */
void test_sawtooth_waveform() {
    S0x16Signal s = sawtooth(constant(1000), s0x16(0));
    ...
    // t=999ms -> ~1
    TEST_ASSERT_INT32_WITHIN(500, S0X16_MAX, raw(s.sample(SIGNED_RANGE, 999)));
}

/** @brief Verify the bounded periodic overload remaps a waveform into a bounded interval. */
void test_bounded_sine_waveform() {
    S0x16Signal s = sine(constant(1000), constant(100), constant(800));
...
void test_bounded_sine_centered_range() {
    S0x16Signal s = sine(constant(1000), constant(250), constant(750));
...
void test_zero_speed_signal() {
    S0x16Signal s = sine(constant(0), s0x16(0));
...
void test_half_speed_signal() {
    ...
    (void) s.sample(SIGNED_RANGE, 200);
    (void) s.sample(SIGNED_RANGE, 400);
    TEST_ASSERT_INT32_WITHIN(500, S0X16_MAX, raw(s.sample(SIGNED_RANGE, 500)));
...
void test_bounded_sine_clamps_and_swaps_bounds() {
    S0x16Signal forward = sine(constant(1000), constant(100), constant(800));
    S0x16Signal reversed = sine(constant(1000), constant(800), constant(100));
    S0x16Signal clamped = sine(constant(1000), constant(1200), constant(1200));
    ...
    TEST_ASSERT_INT32_WITHIN(10, S0X16_MAX, raw(clamped.sample(SIGNED_RANGE, 0)));
}

/** @brief Verify fixed signed phase offsets wrap into the phase domain. */
void test_phase_offset_wraps() {
    S0x16Signal s = sine(constant(1000), s0x16(-16384), constant(0), constant(1000));

    TEST_ASSERT_INT32_WITHIN(500, S0X16_MIN, raw(s.sample(SIGNED_RANGE, 0)));
}
...
void test_smap_animated_bounds() {
    S0x16Signal s = smap(constant(500), linear(1000), constant(1000));
...
void test_smap_preserves_aperiodic_metadata() {
    S0x16Signal s = smap(linear(750, LoopMode::SATURATE), constant(100), constant(800));
...
void test_palette_clip_zero_has_zero_feather() {
    ...
    transform.advanceFrame(u0x16(0), 0);
...
void test_palette_clip_feather_scales_with_clip_signal() {
    ...
    transform.advanceFrame(u0x16(0), 150);
    transform.advanceFrame(u0x16(0), 250);
    ...
    transform.advanceFrame(u0x16(0), 400);
    transform.advanceFrame(u0x16(0), 600);
    transform.advanceFrame(u0x16(0), 750);
    TEST_ASSERT_EQUAL_UINT16(0, raw(context->paletteClip));
    TEST_ASSERT_EQUAL_UINT16(0, raw(context->paletteClipFeather));
}
