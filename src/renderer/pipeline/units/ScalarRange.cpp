...
namespace PolarShader {
    ...
    MappedSignal<int32_t> ScalarRange::map(SFracQ0_16 t) const {
        int64_t span = static_cast<int64_t>(max_scalar) - static_cast<int64_t>(min_scalar);
        if (span == 0) return MappedSignal(min_scalar);
        ...
        return MappedSignal(static_cast<int32_t>(static_cast<int64_t>(min_scalar) + scaled));
    }
}
