...
#include "BoundedAngularModulator.h"
...
namespace PolarShader {
    BoundedAngularModulator::BoundedAngularModulator(
        BoundedAngle initialPhase,
        BoundedScalarSignal speed
    ) : phase_accum(unbindAngle(initialPhase)),
        phase(initialPhase),
        speed(std::move(speed)) {
    }

    void BoundedAngularModulator::advanceFrame(TimeMillis timeInMillis) {
        if (!hasLastTime) {
        ...
        phase_accum = wrapAddSigned(phase_accum, raw(phase_advance));
        phase = bindAngle(phase_accum);
    }
}
