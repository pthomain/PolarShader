...
namespace PolarShader {
    namespace {
        ...
        int32_t floorToIntRaw(int32_t valueRaw) {
            ...
            return -static_cast<int32_t>((-value + S0X16_ONE - 1) >> 16);
        }
        ...
        int32_t sinRadiansRaw(int32_t radiansRaw) {
            return raw(angleSinU0x16(u0x16(angleFromRadiansRaw(radiansRaw))));
        }

        int32_t cosRadiansRaw(int32_t radiansRaw) {
            return raw(angleCosU0x16(u0x16(angleFromRadiansRaw(radiansRaw))));
        }

        uint16_t unsignedCosRadiansRaw(int32_t radiansRaw) {
            return raw(toUnsignedClamped(angleCosU0x16(u0x16(angleFromRadiansRaw(radiansRaw)))));
        }
        ...
        Vec2Q16 shaderUvMin(UV uv, int32_t aspectRaw) {
            ...
            if (aspectRaw >= S0X16_ONE) {
                x = fl::s16x16::from_raw(mulQ16Raw(raw(x), aspectRaw));
            } else if (aspectRaw > 0) {
                y = fl::s16x16::from_raw(divSignedRaw(raw(y), aspectRaw, 32 * S0X16_ONE));
            }
            return Vec2Q16{x, y};
        }
        ...
        uint32_t defaultUnitRaw(uint16_t permille) {
            if (permille >= 1000u) return S0X16_ONE;
            return (static_cast<uint32_t>(permille) * 0xFFFFu) / 1000u + 1u;
        }

        uint32_t signalUnitRaw(const S0x16Signal &signal, TimeMillis elapsedMs, uint16_t fallbackPermille) {
        ...
        uint32_t tanhQ16(uint32_t xRaw) {
            if (xRaw >= static_cast<uint32_t>(8 * S0X16_ONE)) return U0X16_MAX;
            uint32_t twoX = xRaw >= static_cast<uint32_t>(4 * S0X16_ONE)
                ? static_cast<uint32_t>(8 * S0X16_ONE)
                : xRaw << 1;
            uint32_t e = raw(expNegQ16(fl::u16x16::from_raw(twoX)));
            if (e >= static_cast<uint32_t>(S0X16_ONE)) return 0u;
            uint32_t numerator = static_cast<uint32_t>(S0X16_ONE) - e;
            uint32_t denominator = static_cast<uint32_t>(S0X16_ONE) + e;
            uint32_t value = divUnsignedRaw(numerator, denominator, S0X16_ONE);
            return value > U0X16_MAX ? U0X16_MAX : value;
        }

        uint32_t toneMapRocaille(uint32_t accumulatedRaw, uint32_t glowRaw) {
            uint32_t glowing = mulUnsignedRaw(accumulatedRaw, glowRaw);
            if (glowing >= static_cast<uint32_t>(8 * S0X16_ONE)) return U0X16_MAX;
            return tanhQ16(squareRaw(glowing, 64u * S0X16_ONE));
        }
        ...
        uint32_t smoothstepSignedRaw(int32_t edge0Raw, int32_t edge1Raw, int32_t xRaw) {
            if (edge0Raw == edge1Raw) return xRaw < edge0Raw ? 0u : S0X16_ONE;
            ...
            uint32_t shaped = mulUnsignedRaw(t2, static_cast<uint32_t>(3 * S0X16_ONE) - (t << 1));
            return reversed ? static_cast<uint32_t>(S0X16_ONE) - shaped : shaped;
        }

        uint32_t starFieldTravelBirthFadeRaw(int32_t cycleRaw) {
            constexpr int32_t fadeWindowRaw = Q16_0_12;
            int32_t distanceFromBirthRaw = S0X16_ONE - cycleRaw;
            if (distanceFromBirthRaw <= 0) return 0u;
            if (distanceFromBirthRaw >= fadeWindowRaw) return S0X16_ONE;
        ...
        int32_t octBox(Vec3Q16 pos, int32_t scaleRaw) {
            ...
            return -divSignedRaw(base, Q16_1_50, 64 * S0X16_ONE);
        }
        ...
        Vec3Q16 normalize3(Vec3Q16 v) {
            ...
            if (len < 1) len = 1;
            return Vec3Q16{
                fl::s16x16::from_raw(divSignedRaw(raw(v.x), len, 8 * S0X16_ONE)),
                fl::s16x16::from_raw(divSignedRaw(raw(v.y), len, 8 * S0X16_ONE)),
                fl::s16x16::from_raw(divSignedRaw(raw(v.z), len, 8 * S0X16_ONE))
            };
        }
        ...
        uint32_t sqrtUnitRaw(uint32_t valueRaw) {
            ...
            return root > U0X16_MAX ? U0X16_MAX : static_cast<uint32_t>(root);
        }
        ...
        uint32_t acesChannelRaw(uint32_t valueRaw) {
            ...
            return divUnsignedRaw(numerator, denominator, S0X16_ONE);
        }
        ...
        void starFieldColour(uint32_t hash, uint32_t &r, uint32_t &g, uint32_t &b) {
            ...
            switch ((hash >> 29) % 3u) {
                case 0: r = S0X16_ONE; break;
                case 1: g = S0X16_ONE; break;
                default: b = S0X16_ONE; break;
            }
        }
        ...
        int32_t pminRaw(int32_t aRaw, int32_t bRaw, int32_t kRaw) {
            ...
            int32_t soft = mulQ16Raw(kRaw, mulQ16Raw(static_cast<int32_t>(h), S0X16_ONE - static_cast<int32_t>(h)));
            return mixed - soft;
        }
        ...
        int32_t star5Raw(Vec2Q16 point, int32_t radiusRaw, int32_t radiusFactorRaw, int32_t smoothRaw) {
            ...
            Vec2Q16 ba{
                ...
                fl::s16x16::from_raw(mulQ16Raw(radiusFactorRaw, raw(k1.x)) - S0X16_ONE)
            };
            int32_t baLen2 = dot2Raw(ba, ba);
            int32_t projection = baLen2 == 0 ? 0 : divSignedRaw(dot2Raw(p, ba), baLen2, 8 * S0X16_ONE);
    ...
    struct RocaillePattern::State {
        ...
        S0x16Signal lengthSignal;
        S0x16Signal detailSignal;
        S0x16Signal turbulenceSignal;
        S0x16Signal frequencySignal;
        S0x16Signal speedSignal;
        S0x16Signal layersSignal;
        S0x16Signal hueSignal;
        S0x16Signal glowSignal;

        uint32_t timeRaw{0};
        int32_t spatialScaleRaw{19661};
        uint16_t lengthExponentPermille{1000};
        uint8_t detail{9};
        uint32_t turbulenceRaw{S0X16_ONE};
        uint32_t frequencyRaw{S0X16_ONE};
        uint8_t layers{9};
        int32_t hueRaw{0};
        uint32_t glowRaw{S0X16_ONE};
    ...
    struct RocaillePattern::Functor {
        ...
        RgbSample sample(UV uv) const {
            ...
            p.x = fl::s16x16::from_raw(divSignedRaw(raw(p.x), state->spatialScaleRaw, 64 * S0X16_ONE));
            p.y = fl::s16x16::from_raw(divSignedRaw(raw(p.y), state->spatialScaleRaw, 64 * S0X16_ONE));
            ...
            for (uint8_t layerIndex = 1; layerIndex <= state->layers; ++layerIndex) {
                ...
                for (uint8_t iterationIndex = 1; iterationIndex <= state->detail; ++iterationIndex) {
                    ...
                    int32_t common = static_cast<int32_t>(layerIndex) * S0X16_ONE + static_cast<int32_t>(state->timeRaw);
                    ...
                    turbulent.x = fl::s16x16::from_raw(clampRaw(static_cast<int64_t>(raw(turbulent.x)) + stepX, -64 * S0X16_ONE, 64 * S0X16_ONE));
                    turbulent.y = fl::s16x16::from_raw(clampRaw(static_cast<int64_t>(raw(turbulent.y)) + stepY, -64 * S0X16_ONE, 64 * S0X16_ONE));
                }
                ...
                int32_t baseAngle = static_cast<int32_t>(layerIndex) * S0X16_ONE + state->hueRaw;
                uint32_t layerR = static_cast<uint32_t>(unsignedCosRadiansRaw(baseAngle)) / 3u;
                uint32_t layerG = static_cast<uint32_t>(unsignedCosRadiansRaw(baseAngle + S0X16_ONE)) / 3u;
                uint32_t layerB = static_cast<uint32_t>(unsignedCosRadiansRaw(baseAngle + 2 * S0X16_ONE)) / 3u;

                shaderToySaturatingAdd(accR, divUnsignedRaw(layerR, attenuationRaw, 128u * S0X16_ONE));
                shaderToySaturatingAdd(accG, divUnsignedRaw(layerG, attenuationRaw, 128u * S0X16_ONE));
                shaderToySaturatingAdd(accB, divUnsignedRaw(layerB, attenuationRaw, 128u * S0X16_ONE));
            }
    ...
    void RocaillePattern::advanceFrame(u0x16 progress, TimeMillis elapsedMs) {
        (void)progress;
    ...
    struct ProteanCloudsPattern::State {
        ...
        S0x16Signal frequencySignal;
        S0x16Signal brightnessSignal;
        uint32_t timeRaw{0};
        uint32_t warpRaw{S0X16_ONE};
        uint32_t frequencyRaw{Q16_9_00};
        uint32_t brightnessRaw{Q16_0_01};

        State(S0x16Signal speed, S0x16Signal warp, S0x16Signal frequency, S0x16Signal brightness)
            : speedSignal(std::move(speed)),
    ...
    struct ProteanCloudsPattern::Functor {
        ...
        RgbSample sample(UV uv) const {
            ...
            for (uint8_t i = 0; i < 3; ++i) {
                ...
                uint32_t sinPlusOne = static_cast<uint32_t>(sinRadiansRaw(zRaw) + S0X16_ONE);
                ...
                int32_t unitX = divSignedRaw(raw(p.x), static_cast<int32_t>(lRaw), 8 * S0X16_ONE);
                int32_t unitY = divSignedRaw(raw(p.y), static_cast<int32_t>(lRaw), 8 * S0X16_ONE);
                ...
                uint32_t channel = divUnsignedRaw(state->brightnessRaw, tileLength, 64u * S0X16_ONE);
                channels[i] = divUnsignedRaw(channel, lRaw, 64u * S0X16_ONE);
            }
    ...
    void ProteanCloudsPattern::advanceFrame(u0x16 progress, TimeMillis elapsedMs) {
        (void)progress;
    ...
    struct OctgramsPattern::State {
        S0x16Signal speedSignal;
        S0x16Signal travelSignal;
        S0x16Signal pulseSignal;
        S0x16Signal densitySignal;
        S0x16Signal glowSignal;
        uint32_t timeRaw{0};
        uint32_t travelRaw{Q16_4_00};
        uint32_t pulseRaw{S0X16_ONE};
        uint32_t densityRaw{Q16_23_00};
        uint32_t glowRaw{Q16_0_02};

        State(S0x16Signal speed, S0x16Signal travel, S0x16Signal pulse, S0x16Signal density, S0x16Signal glow)
            : speedSignal(std::move(speed)),
    ...
    void OctgramsPattern::advanceFrame(u0x16 progress, TimeMillis elapsedMs) {
        (void)progress;
        state->timeRaw = mulUnsignedRaw(secondsRaw(elapsedMs), signalUnitRaw(state->speedSignal, elapsedMs, 1000));
    ...
    struct RotatingSquaresPattern::State {
        ...
        uint32_t brightnessRaw{S0X16_ONE};

        State(S0x16Signal speed, S0x16Signal thickness, S0x16Signal pulse, S0x16Signal brightness)
            : speedSignal(std::move(speed)),
        ...
        RgbSample sample(UV uv) const {
            ...
            for (uint8_t j = 0; j < 3; ++j) {
                for (uint8_t i = 0; i < 50; ++i) {
                    ...
                    uint32_t fold = static_cast<uint32_t>(S0X16_ONE) - unsignedCosRadiansRaw(phaseAngle);
                    ...
                    int32_t pBase = S0X16_ONE - thRaw * 2;
                    if (pBase < 0) pBase = 0;
                    ...
                    int32_t outer = static_cast<int32_t>(dRaw) - mulQ16Raw(S0X16_ONE - thRaw, static_cast<int32_t>(pRaw));
                    ...
                    if (v1 <= v2) continue;
                    uint32_t vRaw = v1 - v2;
                    uint32_t value = mulUnsignedRaw(vRaw, dRaw > static_cast<uint32_t>(4 * S0X16_ONE) ? static_cast<uint32_t>(8 * S0X16_ONE) : dRaw * 2u);
    ...
    void RotatingSquaresPattern::advanceFrame(u0x16 progress, TimeMillis elapsedMs) {
        (void)progress;
        state->timeRaw = mulUnsignedRaw(secondsRaw(elapsedMs), signalUnitRaw(state->speedSignal, elapsedMs, 1000));
    ...
    struct StarryPlanesPattern::State {
        S0x16Signal speedSignal;
        S0x16Signal planeSpacingSignal;
        S0x16Signal starSizeSignal;
        S0x16Signal pathSignal;
        S0x16Signal brightnessSignal;
        uint32_t timeRaw{0};
        int32_t planeSpacingRaw{Q16_0_50};
        int32_t starRadiusRaw{29491};
        uint32_t pathRaw{S0X16_ONE};
        uint32_t brightnessRaw{S0X16_ONE};
    ...
    struct StarryPlanesPattern::Functor {
        ...
        RgbSample sample(UV uv) const {
            ...
            Vec3Q16 up{
                ddro.x,
                fl::s16x16::from_raw(raw(ddro.y) + S0X16_ONE),
                ddro.z
            };
            ...
            for (uint8_t i = 1; i <= 16; ++i) {
                ...
                int32_t lpdRaw = divSignedRaw(pzRaw - raw(ro.z), rayZ, 64 * S0X16_ONE);
                ...
                uint32_t d2Sq = squareRaw(d2Raw, 32u * S0X16_ONE);
                ...
                uint32_t ring = static_cast<uint32_t>(ringSin + S0X16_ONE) >> 1;
                uint32_t invD2 = divUnsignedRaw(Q16_0_50, d2Sq, 32u * S0X16_ONE);
                uint32_t sparkle = mixUnsignedRaw(invD2, S0X16_ONE, ring);
                uint32_t denominator = d2Sq < static_cast<uint32_t>(2185u) ? 6554u : d2Sq * 3u;
                uint32_t intensity = divUnsignedRaw(sparkle, denominator, 32u * S0X16_ONE);

                int32_t paletteArg = (static_cast<int32_t>(nz + i) * Q16_0_50) + mulQ16Raw(2 * S0X16_ONE, static_cast<int32_t>(d2Raw));
                uint32_t r = mulUnsignedRaw(unsignedCosRadiansRaw(paletteArg - Q16_HALF_PI), intensity);
                uint32_t g = mulUnsignedRaw(unsignedCosRadiansRaw(paletteArg + S0X16_ONE - Q16_HALF_PI), intensity);
                uint32_t b = mulUnsignedRaw(unsignedCosRadiansRaw(paletteArg + 2 * S0X16_ONE - Q16_HALF_PI), intensity);
                ...
                uint32_t remaining = static_cast<uint32_t>(S0X16_ONE) - accA;
                ...
                if (accA > static_cast<uint32_t>(S0X16_ONE)) accA = S0X16_ONE;
            }
    ...
    void StarryPlanesPattern::advanceFrame(u0x16 progress, TimeMillis elapsedMs) {
        (void)progress;
        state->timeRaw = mulUnsignedRaw(secondsRaw(elapsedMs), signalUnitRaw(state->speedSignal, elapsedMs, 1000));
    ...
    struct TrigFieldPattern::State {
        S0x16Signal zoomSignal;
        S0x16Signal yOffsetSignal;
        S0x16Signal waveScaleSignal;
        S0x16Signal speedSignal;
        S0x16Signal colorSpreadSignal;
        S0x16Signal brightnessSignal;
        uint32_t timeRaw{0};
        ...
        int32_t brightnessRaw{S0X16_ONE};
    ...
    struct TrigFieldPattern::Functor {
        ...
        RgbSample sample(UV uv) const {
            ...
            uvY = wrapRadiansRaw(static_cast<int64_t>(uvY) + sinRadiansRaw(divSignedRaw(piRaw, Q16_12_00, 64 * S0X16_ONE) + uvY));
    ...
    void TrigFieldPattern::advanceFrame(u0x16 progress, TimeMillis elapsedMs) {
        (void)progress;
    ...
    struct StarFieldTravelPattern::Functor {
        ...
        void accumulatePlane(
            ...
            int32_t fracX = pxRaw - cellX * S0X16_ONE;
            int32_t fracY = pyRaw - cellY * S0X16_ONE;
            int32_t haloRadiusRaw = state->starSizeRaw * 6;
            uint32_t depthRaw = divUnsignedRaw(S0X16_ONE, static_cast<uint32_t>(zRaw), Q16_3_00);
            ...
            for (int8_t oy = -1; oy <= 1; ++oy) {
                for (int8_t ox = -1; ox <= 1; ++ox) {
                    ...
                    int32_t starXRaw = static_cast<int32_t>(ox) * S0X16_ONE + static_cast<int32_t>(hx & 0xFFFFu);
                    int32_t starYRaw = static_cast<int32_t>(oy) * S0X16_ONE + static_cast<int32_t>(hy & 0xFFFFu);
                    ...
                    if (alphaRaw > static_cast<uint32_t>(S0X16_ONE)) alphaRaw = S0X16_ONE;
        ...
        RgbSample sample(UV uv) const {
            ...
            for (uint8_t layer = 0; layer < layerCount; ++layer) {
                int32_t layerPhaseRaw = static_cast<int32_t>((static_cast<uint32_t>(layer) * S0X16_ONE) / layerCount);
                int32_t cycleRaw = floorModRaw(layerPhaseRaw - timePhaseRaw, S0X16_ONE);
                ...
                for (uint8_t tap = 0; tap < trailTaps; ++tap) {
                    ...
                    uint32_t tapWeightRaw = S0X16_ONE;
                    if (tap > 0) {
                        int32_t tapUnitRaw = static_cast<int32_t>((static_cast<uint32_t>(tap) * S0X16_ONE) / (trailTaps - 1));
                        tapZRaw += mulQ16Raw(static_cast<int32_t>(state->trailLengthRaw), tapUnitRaw);
                        if (tapZRaw > Q16_1_50) continue;
                        uint32_t fadeRaw = (static_cast<uint32_t>(trailTaps - tap) * S0X16_ONE) / trailTaps;
    ...
    void StarFieldTravelPattern::advanceFrame(u0x16 progress, TimeMillis elapsedMs) {
        (void)progress;
