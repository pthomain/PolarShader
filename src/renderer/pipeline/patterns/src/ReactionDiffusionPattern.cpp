...
namespace PolarShader {
    namespace {
        ...
        constexpr uint16_t RD_SATURATION_DELTA = 0x1000;
        constexpr uint8_t RD_SATURATION_STREAK = 10;
    }
    ...
    void ReactionDiffusionPattern::seed(State &s) {
        ...
        s.consecutiveSaturatedFrames = 0;
    }
    ...
    ReactionDiffusionPattern::ReactionDiffusionPattern(
        ...
        state->consecutiveSaturatedFrames = 0;
    ...
    void ReactionDiffusionPattern::advanceFrame(f16 progress, TimeMillis elapsedMs) {
        ...
        const bool saturated = needsReseed(*state);
        if (saturated) {
            if (++state->consecutiveSaturatedFrames >= RD_SATURATION_STREAK) {
                seed(*state);
            }
        } else {
            state->consecutiveSaturatedFrames = 0;
