...
namespace PolarShader {
    ...
    namespace {
        ...
        // ---- atan2 approximation (inlined from PolarMaths.cpp) ----

        constexpr f16 atan2Turns(int16_t y, int16_t x) {
            ...
            uint32_t z = (static_cast<uint32_t>(min_val) << 16) / max_val;
            uint32_t one_minus_z = ANGLE_FULL_TURN_U32 - z;
            constexpr uint32_t A_Q16 = ANGLE_FULL_TURN_U32 / 8;
            constexpr uint32_t B_Q16 = 2847u;
            uint32_t inner = A_Q16 + ((B_Q16 * one_minus_z) >> 16);
            uint32_t base = (z * inner) >> 16;
        ...
        PolarCoord gridToPolar(uint8_t x, uint8_t y, uint8_t gridSize) {
            ...
            f16 angle = atan2Turns(dyScaled, dxScaled);
