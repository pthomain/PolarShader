...
namespace PolarShader {
    constexpr UQ0_16 angleFrac(uint32_t denominator) {
        if (denominator == 0) return UQ0_16(0);
        return UQ0_16(static_cast<uint32_t>((static_cast<uint64_t>(ANGLE_FULL_TURN_U32)) / denominator));
    }

    constexpr UQ0_16 angleFrac360(uint16_t angleDegrees) {
        if (angleDegrees == 0) return UQ0_16(0);
        return UQ0_16(static_cast<uint32_t>((static_cast<uint64_t>(USQ0_16_MAX) * angleDegrees) / 360));
    }

    // FastLED trig sampling helpers (explicit angle/phase conversions).
    constexpr uint16_t angleToFastLedPhase(UQ0_16 a) { return raw(a); }

    // --- Angle wrap arithmetic (mod 2^16) ---
    constexpr UQ0_16 angleWrapAdd(UQ0_16 a, uint16_t delta) {
        return UQ0_16(static_cast<uint16_t>(raw(a) + delta));
    }

    // Wrap-add using signed raw delta (Q0.16), interpreted via two's-complement.
    constexpr UQ0_16 angleWrapAddSigned(UQ0_16 a, int32_t delta_raw_q0_16) {
        uint32_t sum = static_cast<uint32_t>(raw(a)) + static_cast<uint32_t>(delta_raw_q0_16);
        return UQ0_16(static_cast<uint16_t>(sum));
    }

    inline SQ0_16 angleSinQ0_16(UQ0_16 a) {
        int16_t raw_q1_15 = sin16(angleToFastLedPhase(a));
        return SQ0_16(static_cast<int32_t>(raw_q1_15) << 1);
    }

    inline SQ0_16 angleCosQ0_16(UQ0_16 a) {
        int16_t raw_q1_15 = cos16(angleToFastLedPhase(a));
        return SQ0_16(static_cast<int32_t>(raw_q1_15) << 1);
    }

    constexpr UQ0_16 angleAtan2TurnsApprox(int16_t y, int16_t x) {
        if (x == 0 && y == 0) return UQ0_16(0);
        ...
        return UQ0_16(static_cast<uint16_t>(angle & ANGLE_U16_MAX));
    }
}
