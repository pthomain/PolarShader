...
## Context
The spatial pipeline now uses `UV` (two `sr16` values) for sampling. To animate these coordinates effectively (e.g., for translation or custom warp offsets), the signal engine needs to understand how to sample and accumulate `UV` values over time.

## Requirements
- **UVSignal:** Define a new signal type `UVSignal` that maps `TimeMillis` to `UV`.
- **sr16 Accumulation:** Add a `SignalAccumulator` specialization for `sr16` to support relative 16.16 signals.
