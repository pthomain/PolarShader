...
## Context
Currently, the codebase has many range types (`CartRange`, `ZoomRange`, `SFracRange`, etc.) that perform identical linear interpolation but return different units. With the unification of spatial operations under `FracQ16_16` and `UV`, these can be simplified.
