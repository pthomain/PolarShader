...
- [x] Task: Rename `PolarPipeline` to `Layer` and `PolarPipelineBuilder` to `LayerBuilder`.
...
- [x] Task: Update Type Aliases in `src/renderer/pipeline/transforms/base/Layers.h`.
    - Rename `UVLayer` to `UVMap`.
    - Rename `ColourLayer` to `ColourMap`.
- [~] Task: Propagate name changes across the codebase.
...
- [ ] Task: Create `src/renderer/pipeline/signals/interpolators/Interpolator.h`.
    - Define the base `Interpolator` interface with a `calculate(FracQ0_16 progress)` method.
- [ ] Task: Implement `LinearInterpolator` and `QuadraticInterpolator` (In/Out/InOut).
- [ ] Task: Implement `SinusoidalInterpolator` (In/Out/InOut).
- [ ] Task: Implement `ElasticInterpolator` and `BounceInterpolator` (In/Out/InOut).
- [ ] Task: Conductor - User Manual Verification 'Interpolator Hierarchy' (Protocol in workflow.md)

## Phase 3: Animation Signals API
Replace legacy linear/sine/pulse signals with the new `animate` system.

- [ ] Task: Implement `animate(TimeMillis durationMs, const Interpolator& interpolator)` in `Signals.cpp/h`.
- [ ] Task: Remove `sine()` and `pulse()` signal functions from `Signals.h/cpp` and `SignalSamplers.h/cpp`.
- [ ] Task: Update all presets and existing code to use `animate` with appropriate interpolators to replicate previous behavior.
- [ ] Task: Conductor - User Manual Verification 'Animation Signals API' (Protocol in workflow.md)
...
- [ ] Task: Add `alpha` and `BlendMode` to `Layer`.
    - Update `Layer` class and `LayerBuilder` to support these properties.
- [ ] Task: Implement `Scene` class in `src/renderer/pipeline/Scene.h/cpp`.
    - Manage a list of `Layer`s.
    - Handle relative timing (translation from absolute time).
    - Implement composition logic using `BlendMode`.
- [ ] Task: Implement `SceneProvider` interface and `DefaultSceneProvider`.
- [ ] Task: Implement `SceneManager` in `src/renderer/pipeline/SceneManager.h/cpp`.
    - Handle scene lifecycle and duration-based transitions.
- [ ] Task: Update `PolarRenderer` to use `SceneManager`.
- [ ] Task: Conductor - User Manual Verification 'Layer and Scene Architecture' (Protocol in workflow.md)
